...
class QrCodeService {
  final CollectionReference _qrCodeCollection = FirebaseFirestore.instance.collection('qrcodes');
  ...
  Stream<List<QRModel>> getQRCodeStream() {
    return _qrCodeCollection.snapshots().map((snapshot) => snapshot.docs.map((doc) => QRModel.fromJson(doc.data() as Map<String, dynamic>)).toList());
  }
  ...
  Future<List<QRModel>> getQRCodeByUidWithFilter(String uid, DateTime? startDate, DateTime? endDate, {required String reportType}) async {
    try {
      ...
      final qrCodes = querySnapshot.docs.map((doc) => QRModel.fromJson(doc.data() as Map<String, dynamic>)).toList();
