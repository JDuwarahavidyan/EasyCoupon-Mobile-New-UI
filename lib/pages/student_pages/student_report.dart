  ...
  void _fetchData() {
    setState(() {
      ...
      if (_dates[0] != null && _dates[1] != null) {
        ...
        List<String> dateRange = _generateDateRange(_dates[0]!, _dates[1]!);

        // Update the rows to display the selected dates under "Date and Time"
        rows = dateRange.map<DataRow>((date) {
          bool isEven = dateRange.indexOf(date) % 2 == 0;
          return DataRow(
            ...
            cells: [
              DataCell(Center(child: Text(date, style: _cellStyle))),
              DataCell(Center(
                  child: Text(dummyData[0]["Canteen"], style: _cellStyle))),
              DataCell(Center(
                  child: Text(dummyData[0]["Coupons Used"].toString(),
                      style: _cellStyle))),
            ],
          );
        }).toList();
